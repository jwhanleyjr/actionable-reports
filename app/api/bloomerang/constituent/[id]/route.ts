...
export async function GET(_request: NextRequest, { params }: { params: { id: string } }) {
  ...
  const url = new URL(`https://api.bloomerang.co/v2/constituent/${accountId}`);
  ...
  for (const mode of modes) {
    ...
    console.log('Bloomerang constituent response', {
      url: url.toString(),
  ...
  return NextResponse.json({
    ok: false,
    url: url.toString(),
    error: 'Unable to complete Bloomerang constituent lookup.',
  }, { status: 502 });
}
